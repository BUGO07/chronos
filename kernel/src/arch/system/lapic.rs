...
pub fn init() {
    ...
    unsafe { std::sched::LAPIC_ARM.set(arm).unwrap() };
...
fn lapic_oneshot_timer_handler(stack_frame: *mut StackFrame) {
    std::sched::schedule(stack_frame);
...
fn mmio_write(reg: u32, val: u32) {
    ...
    std::asm::mmio::write(addr, val as u64, core::mem::size_of::<u32>());
}

#[inline(always)]
fn mmio_read(reg: u32) -> u32 {
    let addr = MMIO.load(Ordering::Relaxed) + reg as u64;
    std::asm::mmio::read(addr, core::mem::size_of::<u32>()) as u32
}
