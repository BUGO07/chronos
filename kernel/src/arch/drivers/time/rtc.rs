...
impl RtcTime {
    ...
    pub fn to_epoch(&self) -> Option<u64> {
        ...
        for year in 1970..self.year {
            days += if std::time::is_leap_year(year) {
                366
            } else {
                365
            };
        ...
        for month in 1..self.month {
            days += std::time::days_in_month(self.year, month) as u64;
        }
        if self.day as u32 > std::time::days_in_month(self.year, self.month) {
            return None;
        }
